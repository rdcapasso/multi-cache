...
class FileCache implements CacheInterface
{
    ...
    public function get($cacheKey)
    {
        ...
        if ($isValid === true) {
            ...
            $filename = $cacheKey . ".cache" . ($this->use_compression ? ".gz" : "");
    ...
    public function set($cacheKey, $data, $expires = 0)
    {
        ...
        //set filename.
        $filename = $this->cache_dir . "/" . ($this->use_compression ? $cacheKey . ".cache.gz" : $cacheKey . ".cache");
    ...
    {
        //assumption is that the cachekey exists.  nonexistent cachekey is exception-worthy.
        if ($this->isValid($cacheKey) === false)
            Throw new FileCacheException("$cacheKey does not exist in cache!");

        //delete the file
        $filename = $this->cache_dir . "/" . $cacheKey . ".cache" . ($this->use_compression === true ? ".gz" : "");
        unlink($filename);
    ...
    public function read($cacheKey)
    {
        ...
        if ($isValid !== false) {
            ...
            $returnVal = array("expires" => $this->cache_contents[$cacheKey],
                "size" => filesize($this->cache_dir . "/" . $cacheKey . ".cache" . ($this->use_compression !== false ? ".gz" : "")),
                "dirname" => $this->cache_dir,
                "filename" => $cacheKey . ".cache" . ($this->use_compression !== false ? ".gz" : ""),
            );
        }
    ...
    public function getCacheSize()
    {
        ...
        pclose($io);
        return $size;
        //        return system('du -sk '.$this->cache_dir,$r);
    }
